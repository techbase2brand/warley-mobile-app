...
export const CartProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const removeOneFromCart = useCallback(
    async (lineId: string) => {
      ...
      console.log("Cart Line ID to be updated:", lineId);
      ...
      try {
        // Retrieve the current cart line details to get the latest quantity
        const { data: cartData } = await fetchCart({
          variables: {
            cartId: id,
            first: 10, // Ensure pagination works correctly
          },
        });
  
        const line = cartData.cart.lines.edges.find((edge) => edge.node.id === lineId);
        const currentQuantity = line?.node.quantity || 0;
  
        console.log("Current Quantity:", currentQuantity);
  
        if (currentQuantity > 1) {
          const newQuantity = currentQuantity - 1; // Decrement by 1
          ...
          console.log(`Updated quantity of line ${lineId} to ${newQuantity}`);
        } else {
    ...
    [cartId, cartLinesUpdate, fetchCart]
  );
