...
const ForgetPasswordScreen = ({ navigation }: { navigation: any }) => {
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      const response = await fetch('https://warley-thv5m.ondigitalocean.app/api/forgotPassword', {
        method: 'POST',
        headers: {
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      const response = await fetch('https://warley-thv5m.ondigitalocean.app/api/resetPassword', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
